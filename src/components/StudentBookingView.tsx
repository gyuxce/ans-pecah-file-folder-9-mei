...
import { CalendarDays, Check, Clock3, Loader2, Search, X } from 'lucide-react';
...
export const StudentBookingView = () => {
  ...
  const endDate = format(addDays(new Date(), 20), 'yyyy-MM-dd');
  ...
  const loadSlots = useCallback(async () => {
    ...
    const { data, error } = await supabase.rpc('get_available_booking_slots', {
      ...
      p_end_date: endDate,
      p_sensei_id: senseiFilter === 'all' ? null : senseiFilter
    });
    if (error) toast.error(`Gagal memuat jadwal: ${error.message}`);
    setSlots((data || [])
    ...
    setLoading(false);
  }, [endDate, senseiFilter, startDate, supabase]);
  ...
  const grouped = useMemo(() => {
    ...
    slots.forEach(slot => map.set(slot.slot_date, [...(map.get(slot.slot_date) || []), slot]));
    return [...map.entries()];
  }, [slots]);
          ...
          {grouped.map(([date, dateSlots]) => (
            <section key={date} className="ui-panel overflow-hidden">
              <div className="border-b border-slate-100 px-4 py-3 dark:border-slate-800">
                <p className="text-sm font-bold text-slate-900 dark:text-white">{format(parseISO(date), 'EEEE, dd MMMM yyyy')}</p>
              </div>
